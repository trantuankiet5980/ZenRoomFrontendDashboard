  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="text-sm font-medium text-slate-700">Thống kê bài đăng</div>
          <div className="text-xs text-slate-500">Tổng: {formatNumber(totalPosts)} bài</div>
          {periodLabel ? (
        ...
        {chartData.length ? (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 10, right: 24, left: 0, bottom: 10 }}>
              ...
              />
              <Tooltip
                formatter={(value) => [formatNumber(value), "Bài đăng"]}
          ...
          <div className="flex h-full items-center justify-center text-sm text-slate-500">
            {loading ? "Đang tải dữ liệu bài đăng…" : "Chưa có dữ liệu"}
          </div>
        )}
      </div>
      ...
      <div className="mt-4">
        <div className="mb-2 text-sm font-medium text-slate-700">
          {period === "YEAR" ? "Bài đăng theo tháng" : "Bài đăng theo ngày"}
        </div>
        {breakdownList.length ? (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 text-sm">
            {breakdownList.map((item) => (
              <div key={item.key} className="text-xs">
                <div className="font-semibold">{formatNumber(item.totalPosts)}</div>
